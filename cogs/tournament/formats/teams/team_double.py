...
def make_matchups(team_names: list) -> list:
    matchups = []
    for i in range(0, len(team_names), 2):
        if i + 1 < len(team_names):
            matchups.append({"player1": team_names[i], "player2": team_names[i + 1]})
        else:
            matchups.append({"player1": team_names[i], "player2": "BYE"})
    return matchups


async def send_match(thread, tournament_name: str, bracket: str, round_index: int, match_index: int, t1: str, t2: str, teams: list, round_label: str):
    color = discord.Color.green() if bracket == "winners" else discord.Color.red() if bracket == "losers" else discord.Color.gold()

    cap1 = get_captain(t1, teams)
    cap2 = get_captain(t2, teams)
    cap1_mention = f"<@{cap1['id']}>" if cap1 else t1
    cap2_mention = f"<@{cap2['id']}>" if cap2 else t2

    embed = discord.Embed(
        title=f"{round_label} - Match {match_index + 1}: {t1} vs {t2}",
...
async def start(interaction: discord.Interaction, tournament_name: str, tournament: dict, teams: list, thread):
    ...
    matchups = make_matchups(team_names)
    ...
    for i, match in enumerate(matchups):
        await send_match(thread, tournament_name, "winners", 0, i, match["player1"], match["player2"], teams, "Winners R1")
...
async def advance(interaction: discord.Interaction, tournament: dict, tournament_name: str, bracket: str, round_index: int, winner_name: str, loser_name: str):
    ...
    if bracket == "winners":
        await _advance_winners(interaction, tournament, tournament_name, round_index, winner_name, loser_name, thread, teams)
    elif bracket == "losers":
        await _advance_losers(interaction, tournament, tournament_name, round_index, winner_name, loser_name, thread, teams)
    elif bracket == "grand_final":
        await _advance_grand_final(interaction, tournament, tournament_name, winner_name, loser_name, thread, teams)
    elif bracket == "grand_final_reset":
        await _advance_grand_final_reset(interaction, tournament, tournament_name, winner_name, thread)


async def _advance_winners(interaction, tournament, tournament_name, round_index, winner_name, loser_name, thread, teams):
    ...
    if len(winners) == 1:
        ...
        await _try_start_grand_final(interaction, tournament, tournament_name, thread, teams)
        return
    ...
    await _start_losers_round(tournament, tournament_name, thread, teams)

    await thread.send(f"**Winners Bracket - Round {next_round_index + 1}**")
    for i, match in enumerate(next_matchups):
        await send_match(thread, tournament_name, "winners", next_round_index, i, match["player1"], match["player2"], teams, f"Winners R{next_round_index + 1}")

    await interaction.response.send_message(f"Match recorded: **{winner_name} wins!** Next round posted.")


async def _start_losers_round(tournament, tournament_name, thread, teams):
    ...
    for i, match in enumerate(matchups):
        await send_match(thread, tournament_name, "losers", round_index, i, match["player1"], match["player2"], teams, f"Losers R{round_index + 1}")


async def _advance_losers(interaction, tournament, tournament_name, round_index, winner_name, loser_name, thread, teams):
    losers_bracket = tournament["losers_bracket"]
    ...
    for i, match in enumerate(next_matchups):
        await send_match(thread, tournament_name, "losers", next_round_index, i, match["player1"], match["player2"], teams, f"Losers R{next_round_index + 1}")

    await interaction.response.send_message(f"Match recorded: **{winner_name} wins!** Next round posted.")


async def _try_start_grand_final(interaction, tournament, tournament_name, thread, teams):
    ...
    cap1 = get_captain(winners_champ, teams)
    cap2 = get_captain(losers_champ, teams)
    cap1_mention = f"<@{cap1['id']}>" if cap1 else winners_champ
    cap2_mention = f"<@{cap2['id']}>" if cap2 else losers_champ
...
async def _advance_grand_final(interaction, tournament, tournament_name, winner_name, loser_name, thread, teams):
    winners_champ = tournament.get("winners_champion")
    ...
    if winner_name == winners_champ:
        ...
        cap1 = get_captain(winners_champ, teams)
        cap2 = get_captain(winner_name, teams)
        cap1_mention = f"<@{cap1['id']}>" if cap1 else winners_champ
        cap2_mention = f"<@{cap2['id']}>" if cap2 else winner_name
        ...
        await thread.send(f"**Grand Final Reset!** Both teams are now 1-1. One final match!")
        await thread.send(f"{cap1_mention} vs {cap2_mention}", embed=embed, view=view)
